...
int main()
{
    ...
    while (1)
    {
        ...
        double pedal_pos = adc_convt(adc_get_data(adc_pedal), gc_adc_app_res, gc_adc_app_vref);
        // measure throttle plate position
        double throttle_pos = adc_convt(adc_get_data(adc_throttle), gc_adc_throttle_res, gc_adc_throttle_vref);
