...
class sc_tlm_bus : public sc_core::sc_module
{
  public:
    ...
    SC_CTOR(sc_tlm_bus) : data_bus("data_bus")
    {
        ...
        {
            initiator_socket[i] =
                new tlm_utils::simple_initiator_socket_tagged<sc_tlm_bus>(sc_core::sc_gen_unique_name("socket"));
        }
    }
  ...
  private:
    ...
    inline unsigned int decode_address(sc_dt::uint64 address, sc_dt::uint64& masked_address)
    {
        ...
        LOG_VERBOSE(
            "Decoding address 0x%4X to masked address = 0x%04X, target = %d ", address, masked_address, target_nr);
