...
class sc_tlm_ecu_iss : public sc_core::sc_module
{
  public:
    ...
    sc_tlm_ecu_iss(sc_core::sc_module_name name, const char* configFile, const char* imageFile);
  ...
  private:
    ...
    int bus_read(unsigned long int addr, unsigned char mask[], unsigned char rdata[], int dataLen);

    int bus_write(unsigned long int addr, unsigned char mask[], unsigned char wdata[], int dataLen);
};
