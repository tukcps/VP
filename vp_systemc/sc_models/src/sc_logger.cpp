...
void sc_logger::log(const char* log_cat, const char* moddel_name, const char* format, ...)
{
    ...
    log_file_ << left << setw(7) << setfill(' ') << log_cat << " | " << right << setw(15) << setfill(' ')
              << sc_core::sc_time_stamp().value() / 1000 << " ns"
              << " | " << left << setw(30) << setfill(' ') << moddel_name << " | " << buffer << endl;
    log_file_.flush();
    va_end(ap);
}
...
sc_logger& sc_logger::operator<<(const string& message)
{
    ...
    log_file_ << left << setw(7) << setfill(' ') << "LOG_INFO"
              << " | " << right << setw(15) << setfill(' ') << sc_core::sc_time_stamp().value() / 1000 << " ns"
              << " | " << message << "\n";
    return *this;
#endif
}
