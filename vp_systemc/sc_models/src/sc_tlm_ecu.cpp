...
// ----------------------------------------------------------------------------
sc_tlm_ecu::sc_tlm_ecu(sc_core::sc_module_name nm) : sc_module(nm), data_bus("bus_initiator")
{
...
// ----------------------------------------------------------------------------
void sc_tlm_ecu::do_trans(tlm::tlm_generic_payload& trans)
{
...
void sc_tlm_ecu::program_main()
{
    ...
    while (true)
    {
        ...
        double pedal_pos = adc_convt(adc_get_data(adc_pedal), k_adc_app_res, k_adc_app_vref);

        double throttle_pos = adc_convt(adc_get_data(adc_throttle), k_adc_throttle_res, k_adc_throttle_vref);
...
double sc_tlm_ecu::pid_calc(const double& tarVal, const double& currVal, const double& pid_za, const double& pid_zb)
{
    gsLastErr = gsCurrErr;
...
// -----------------------------------------------------------------------------
double sc_tlm_ecu::adc_convt(int16_t readout, unsigned int res, double refvol)
{
...
void sc_tlm_ecu::pwm_send_duty(double& duty)
{
    ...
    // saturation
    if (pwm_duty > k_pwm_steps)
        pwm_duty = k_pwm_steps;
