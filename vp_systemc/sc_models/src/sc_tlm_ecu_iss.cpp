...
// ----------------------------------------------------------------------------
sc_tlm_ecu_iss::sc_tlm_ecu_iss(sc_core::sc_module_name name, const char* configFile, const char* imageFile)
    :
...
//! @param name        module name
// ----------------------------------------------------------------------------
sc_tlm_ecu_iss::sc_tlm_ecu_iss(sc_core::sc_module_name name) : sc_module(name), data_bus("data_initiator")
{
...
// ----------------------------------------------------------------------------
void sc_tlm_ecu_iss::run()
{
    while (true)
    {
        sc_core::sc_time time_left = g_quantum->compute_local_quantum() - iss_qkeeper.get_local_time();
...
//! @return  Zero on success. A return code otherwise.
// ----------------------------------------------------------------------------
int sc_tlm_ecu_iss::bus_read(unsigned long int addr, unsigned char mask[], unsigned char rdata[], int dataLen)
{
...
//! @return  Zero on success. A return code otherwise.
// ----------------------------------------------------------------------------
int sc_tlm_ecu_iss::bus_write(unsigned long int addr, unsigned char mask[], unsigned char wdata[], int dataLen)
{
    // Set up the payload fields. Assume everything is 4 bytes.
    trans.set_write();
    trans.set_address((sc_dt::uint64)addr);
